...
Two things get cut. The tape sleeps a fixed stretch after Enter so a slow run
...
coding agent that prints nothing until it is done. Nothing between the first
and last visible change is dropped, and the millisecond stamps printed on
screen are whatever the run measured.
...
def kept_frames(frames, max_idle=MAX_IDLE_SECONDS):
    ...
    for index in range(1, len(marks)):
        if marks[index] != marks[index - 1]:
            last_change = index
    end = min(len(frames), last_change + 1 + round(TAIL_HOLD_SECONDS * CAPTURE_FPS))
