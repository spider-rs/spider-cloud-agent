...
one run."
    ...
    Router(RouterArgs),
    /// Store how the parameter optimizer runs.
    #[command(
        long_about = "Store how the parameter optimizer runs, in ~/.spider/optimize.json.
...
shadow scores and changes nothing. apply writes the pick onto the fields you \
left unset. Without weights the scorer abstains, so apply edits nothing: name \
a weights file with --model, or with --optimize-model for one run.

Every command that fetches pages reads this file. --optimize and \
--optimize-model override it for one run, and --no-optimize, or \
SPIDER_AGENT_NO_OPTIMIZE set to any value, leaves the optimizer out. Local, no \
call."
    )]
...
pub struct Global {
    ...
    /// Run the parameter optimizer on the first attempt of each page. It never
    /// edits a field you set. Left alone, the stored settings decide.
    #[arg(long, global = true, value_name = "MODE", value_parser = optimizer_modes(), conflicts_with = "no_optimize")]
    pub optimize: Option<Mode>,

    /// The weights the optimizer scores with. Without them it abstains, and
    /// apply edits nothing.
    #[arg(
        long,
        global = true,
        value_name = "FILE",
        conflicts_with = "no_optimize"
    )]
    pub optimize_model: Option<String>,

    /// Leave the optimizer out of this run, the stored settings included. The
    /// same as setting SPIDER_AGENT_NO_OPTIMIZE.
    #[arg(long, global = true)]
...
/// A `--mode` value.
///
/// Three flags are spelled `--mode`: the global one, for how a page is
/// fetched, the one on `router set`, for how a provider takes part, and the one
/// on `optimize set`, for what the optimizer does with its pick. clap keeps one
/// slot per name and hands a value on a subcommand up to the global slot, so
/// all three parse into this one type, and each accepts only its own names.
...
pub enum Mode {
    ...
    First,
    /// No outside provider, and the optimizer left out of the request.
    Off,
    /// The optimizer scores and records, and changes nothing.
    Shadow,
    /// The optimizer writes its pick onto the fields the caller left unset.
    Apply,
}
...
/// `--optimize` and `optimize set --mode`: what the optimizer does with its
/// pick.
fn optimizer_modes() -> impl clap::builder::TypedValueParser<Value = Mode> {
    use clap::builder::{PossibleValue, PossibleValuesParser, TypedValueParser};
    PossibleValuesParser::new([
        PossibleValue::new("off").help("Send the request as the router left it"),
        PossibleValue::new("shadow").help("Score and record, and change nothing"),
        PossibleValue::new("apply").help("Write the chosen edits onto the fields you left unset"),
    ])
    .try_map(|name| match name.as_str() {
        "off" => Ok(Mode::Off),
        "shadow" => Ok(Mode::Shadow),
        "apply" => Ok(Mode::Apply),
        other => Err(format!("{other} is not an optimizer mode")),
    })
}
...
pub enum OptimizeCommand {
    /// Store or change the settings. A flag you do not pass keeps its stored
    /// value.
    Set(OptimizeSetArgs),
...
/// Store or change how the optimizer runs.
#[derive(Debug, Args)]
pub struct OptimizeSetArgs {
    /// What to do with the pick.
    #[arg(long, value_name = "MODE", value_parser = optimizer_modes())]
    pub mode: Option<Mode>,

    /// The weights file to score with. It is read on every run that fetches a
    /// page, so keep it where it is.
    #[arg(long, value_name = "FILE", conflicts_with = "no_model")]
    pub model: Option<String>,

    /// Forget the stored weights file. The mode is left as it is.
    #[arg(long)]
    pub no_model: bool,
}
