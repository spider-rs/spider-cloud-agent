...
///
/// Nothing here is a key. The mode and the path to the weights are what was
/// stored, and a run that names either on the command line does not change
/// what this prints.
pub fn optimize(stored: Option<&crate::optimize::StoredOptimize>) -> Item {
    let Some(stored) = stored else {
        return Item::structured(json!({ "type": "optimize", "stored": false }))
            .with_text("stored\tfalse".to_string(), "txt");
    };
    let mode = stored.mode().as_str();
