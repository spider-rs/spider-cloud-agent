...
static OPTIMIZER_NOTED: AtomicBool = AtomicBool::new(false);
...
fn build_client(global: &Global, credits: Option<f64>, fetching: Option<Log>) -> Run<Spider> {
    ...
    // The weights are read here too, so a file that cannot be used stops the
    // run before the first page is paid for.
    #[cfg(feature = "optimize")]
    let mut optimizer_note: Option<String> = None;
    if fetching.is_some() {
        let settings = crate::optimize::settings(global)?;
        #[cfg(feature = "optimize")]
        if let Some(ready) = crate::optimize::build(&settings)? {
            builder = builder.optimizer(ready.optimizer);
            optimizer_note = Some(ready.note);
        }
        #[cfg(not(feature = "optimize"))]
        crate::optimize::build(&settings)?;
    }
    ...
    #[cfg(feature = "optimize")]
    if let (Some(log), Some(note)) = (fetching, optimizer_note) {
        if !OPTIMIZER_NOTED.swap(true, Ordering::Relaxed) {
            log.say(note);
        }
    }
    Ok(spider)
...
pub fn mode(global: &Global) -> Option<RequestMode> {
    // The router modes and the optimizer modes share the type and never parse
    // on this flag.
    global.mode.and_then(|mode| match mode {
        ...
        Mode::Fallback | Mode::First | Mode::Off | Mode::Shadow | Mode::Apply => None,
    })
}
