#!/usr/bin/env bash
# Both feature configurations must keep the normal dependency closure local.
set -euo pipefail
cd "$(dirname "$0")/.."
python3 - <<'PY'
import importlib
...
count, errors = boundary.scan('spider-route/src')
# Explicit reviewed closure of serde and url, including their proc macros.
...
for flags in ([], ['--all-features']):
    ...
    roots = [p['id'] for p in data['packages'] if p['name'] == 'spider-route']
    if len(roots) != 1:
        errors.append('expected exactly one spider-route package')
        continue
    seen = set()
    pending = [roots[0]]
    while pending:
    ...
    if not {'serde', 'url'} <= names or len(seen) < 2:
...
for error in errors:
    print(error)
print(f'route boundary: checked {count} files, {dependencies} dependencies across {configs} configurations')
