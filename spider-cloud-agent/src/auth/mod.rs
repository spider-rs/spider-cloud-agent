...
//!
//! Two pieces. [`store`] finds a key that is already on the machine and writes
//! one back, sharing both places with the command line tools so signing in with
//! either tool signs you in for all of them. [`oauth`], behind the `oauth`
//! feature, opens a browser and comes back with a freshly provisioned key.
//!
